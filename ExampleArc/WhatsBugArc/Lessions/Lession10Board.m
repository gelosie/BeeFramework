...
#import "Lession10Board.h"

#pragma mark -

@implementation Lession10Board
...
- (void)load
{
	...
	_model = [[Lession10Model alloc] init];
}
...
- (void)handleUISignal_BeeUIBoard:(BeeUISignal *)signal
{
	...
	{
		[self setTitleString:@"Lession 10"];
		...
		[_button1 addSignal:Lession10Board.BUTTON1_TOUCHED forControlEvents:UIControlEventTouchUpInside];
		...
		[_button2 addSignal:Lession10Board.BUTTON2_TOUCHED forControlEvents:UIControlEventTouchUpInside];
...
- (void)handleUISignal_Lession10Board:(BeeUISignal *)signal
{
	[super handleUISignal:signal];

	if ( [signal is:Lession10Board.BUTTON1_TOUCHED] )
	{
		self.title = @"Loaded";
	...
	else if ( [signal is:Lession10Board.BUTTON2_TOUCHED] )
	{
		self.title = @"Saved";
